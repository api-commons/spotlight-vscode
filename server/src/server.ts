 ...
 * Spotlight ruleset, which is assumed to be in the same folder as the document.
...
/**
 * Determines the default Spotlight ruleset, which is assumed to be in the workspace folder.
...
function resolveSettings(document: TextDocument): Thenable<TextDocumentSettings> {
  ...
  // its middleware. The client will actually retrieve the 'spotlight' section itself.
...
/**
 * Validates a single document with Spotlight and presents the results to VS Code as diagnostics.
