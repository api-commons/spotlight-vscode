...
/**
 * Wrapper for the Spotlight linter that runs linting against VS Code document
...
export class Linter {
  ...
  /**
   * Executes Spotlight linting against a VS Code document.
  ...
  public async lint(document: TextDocument, ruleset: Ruleset | undefined): Promise<IRuleResult[]> {
    // Unclear if we may have issues changing the ruleset on the shared Spotlight
    ...
    // It's unclear why JSON and YAML both get parsed as YAML, but that's how Spotlight does it, sooooooo...
