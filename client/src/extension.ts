...
/**
 * The set of FileSystemWatcher objects that monitor Spotlight configuration
...
export function activate(context: ExtensionContext): void {
      ...
      workspace: {
        ...
        configuration: async (params, _token, _next): Promise<any[]> => {
          ...
          for (const item of params.items) {
            ...
            const config = Workspace.getConfiguration('spotlight', resource);
  ...
  // server.
  try {
    client = new LanguageClient('spotlight', 'Spotlight', serverOptions, clientOptions);
  } catch (err) {
    Window.showErrorMessage(`The Spotlight extension couldn't be started. See the Spotlight output channel for details.`);
    return;
  }
  ...
  commands = [Commands.registerCommand('spotlight.showOutputChannel', () => {
