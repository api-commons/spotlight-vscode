...
export const activate = async (): Promise<void> => {
  ...
  const ext = vscode.extensions.getExtension('api-commons.spotlight');
...
export const setRulesetFile = (rulesetFile: string): void => {
  vscode.workspace.getConfiguration('spotlight')
...
export const retrieveOutputChannelId = async (): Promise<vscode.Uri> => {
  await vscode.commands.executeCommand('spotlight.showOutputChannel');
  ...
  if (outputs.length !== 1) {
    throw new Error('Unable to retrieve the Spotlight output channel');
  }
